...
foreach($listDir as $f){
  ...
  $barcode = $PDO->query("SELECT * FROM document WHERE case_id='" . $case_id . "' AND type='incoming'" . $exclude . " AND `name`='38' ORDER by id DESC")->fetch();
  $bar_check[$case_id][] = $barcode["id"];

  $Barcode = new \plugin\Document\php\Barcode();
  $content = file_get_contents($listConv . '/' . $name);
  ?>
    <div class="grao-print relative">
      <div id="barcode-<?php echo $cnt;?>" class="grao-barcode relative"><?php $Barcode->_($barcode, "barcode-" . $cnt);?></div>
      <div class="grao-content relative"><?php echo $content;?></div>
    </div>
  <?php
}
?>
