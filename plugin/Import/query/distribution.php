...
foreach ($distribution as $a){ // SAVE DISTRIBUTIONS DATA
	$case_id = $Query->select( ($_POST["case_id" . $a] ? $_POST["case_id" . $a] : 0), 'number', 'caser', 'id')["id"];
	$case = ($case_id != "") ? $case_id : 0;
	$user_id = $Query->select( ($_POST["user" . $a] ? $_POST["user" . $a] : 0), 'email', $User->table,  'id')["id"];
	...
	for($c = 0; $c < 10; ++$c){
		if(isset($_POST[$c . "creditor" . $a])){
			$creditor_id = $Query->select($_POST[$c . "creditor" . $a], "name", "person", "id")["id"];			
