...
for($a = 1; $a <= $_POST["payment-cnt"]; ++$a){
	if(isset($_POST["payment_" . $a])){
		...
		\system\Data::update(array("case_id" => $_POST["case_id"]), $_POST["payment_" . $a], "id", "payment");
	}
}
if(count($mass) > 1){
	$array = array();
	...
	foreach($mass as $id){
		\system\Data::update($array, $id, "id", "invoice");
	}
} else {
	$array = array(
	...
	$array["payment"] = json_encode($payments);
	\system\Data::update($array, $_GET["id"], "id", "invoice");
}
?>
