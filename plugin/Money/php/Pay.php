...
class pay{
	...
	public function splitName($names){
		$name = explode(" ", $names);
		$newName = $name[0];
	...
	public function open_sledene($creditor){
		return '<a href="' . $this->Page->url(21) . '?creditor=' . $creditor . '" target="_blank">Следене за плащания</a>';
	}
	
	public function sledene($case_id, $months=6){
		$pay = $this->PDO->query("SELECT date FROM distribution WHERE case_id='" . $case_id . "' ORDER by date DESC")->fetch();
		$nap = $this->PDO->query("SELECT date FROM document WHERE case_id='" . $case_id . "' AND name='15' ORDER by date DESC")->fetch();
		$noi = $this->PDO->query("SELECT date FROM document WHERE case_id='" . $case_id . "' AND (name='30' OR name='93' OR name='280') ORDER by date DESC")->fetch();
		$bnb = $this->PDO->query("SELECT date FROM document WHERE case_id='" . $case_id . "' AND name='222' ORDER by date DESC")->fetch();
	...
	public function caserNumbersDescription($distribution, $creditor_name){
		if(strpos($creditor_name, "ТД НАП ПЛОВДИВ") === false){
			$Caser = new \plugin\Caser\php\Caser($distribution["case_id"]);
	...
	public function budgetPay($creditorID, $creditor_name, $a, $sum, $egn, $distribution){
		$accounts = $this->PDO->query("SELECT * FROM bank WHERE person_id=" .$creditorID . " ORDER by id ASC");
		?>
		<td class="iban">
			...
			<div class="budgetIBAN">
				...
				<div class="inline-block">
					...
					<option value="0">SELECT</option>
						<?php foreach($this->PDO->query("SELECT * FROM bank_units ORDER by name ASC") as $bank_unit){ ?>
					...
					<textarea type="text" name="<?php echo $n . '_budget_text' . $a;?>" class="budgetTexts <?php echo 'budgetRow'.  $n . '_' . $a;?>" maxlength="70" disabled="disabled"><?php echo $bank["description"] . $this->caserNumbersDescription($distribution, $creditor_name);?></textarea>
				</div>
			</div>
		<?php
		$n++;
	...
	public function sum_format($sum){
