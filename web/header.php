...
<div id="header-searches" class="head-field">
	<div class="head-opener">
		...
		<button class="button" onclick="if(S('#headCaseChoose').value != ''){window.open('<?php echo \system\Core::url() . "Caser/open?id=";?>' + S('#headCaseChoose').value, '_blank');} else {S.info('Не е намерено изпълнително дело!');}">Дело</button>
	</div>

	<form onsubmit="if(S('#header_search').value == ''){ return false;}; return S.popup('<?php echo \system\Core::url() . 'query/header-search';?>', {search: S('#header_search').value})">
