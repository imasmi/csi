...
$check_email = $PDO -> query("SELECT * FROM " . \system\Query::table() . " WHERE email='" . $_POST["email"] . "'");
if($check_email->rowCount() === 0){echo $Text->_("This email is not registred"); exit;}

$new_pass = uniqid();
#CHANGE PASSWORD TO USER
$update_password = $PDO->prepare("UPDATE " . \system\Query::table() . " SET password = ? WHERE email LIKE '" . $_POST["email"] . "'");
