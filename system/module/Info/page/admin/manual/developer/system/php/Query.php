<h1>Query.php</h1>
    ...
    <h3>Query->select</h3>
        <span>Select database row from specified table and return the output as array.</span>
        <ul>
            <li>
                <h4>Query->select(selector value, selector column="id", table="module", columns="*", selector type="=")</h4>
                <span>selector value: specified value to select from the table. Example: 3 (will select something with value 3 in it)</span>
                ...
                <span>columns (optional): columns to be fetched from the table. If left unfilled all columns will be fetched. Example: id, name (will fetch only id and name columns from the specified table)</span>
                ...
                <p>Example: $Query->select("3", "id", "web_page");</p>
                <span>Will output the first finded result from the following query: SELECT * FROM web_page WHERE id='3'.</span>
                <p>Example: $Query->select("homepage", "type", "web_page", "id, tag", "LIKE").</p>
                <span>Will output the first finded result from the following query: SELECT id, tag FROM web_page WHERE type LIKE 'homepage'.</span>
            </li>
        </ul>
    <h3>Query->insert</h3>
        ...
        <ul>
            <li>
                ...
                <p>Example: $Query->insert(array("tag" => "web", "type" => "homepage"), "web_page");</p>
    ...
    <h3>Query->update</h3>
        ...
        <ul>
            <li>
                ...
                <p>Example: $Query->update(array("tag" => "web", "type" => "homepage"), "3", "id", "web_page");</p>
                <span>Will update a row with id = '3' in web_page table with web value in column tag and homepage value in column type.</span>
                <p>Example: $Query->update(array("tag" => "web", "type" => "homepage"), "new", "type", "web_page", "LIKE");</p>
    ...
    <h3>Query->remove</h3>
        <span>Delete database row from specified table.</span>
        <ul>
            <li>
                <h4>Query->remove(selector value, selector column="id", table="module", selector type="=")</h4>
                <span>selector value: specified value to select row to be deleted in the table. Example: 3 (will select something with value 3 in it)</span>
                <span>selector column (optional): the column where the selector value to be searched. Default is id. Example: age (will search for the selector value in column with name - age)</span>
                <span>table (optional): the table to select in. If left unfilled and the current url location is in some module or plugin, the system will automatically resolve the default module or plugin table. Example: web_page (will search for the selector value in table web_page)</span>
                <span>selector type (optional): the symbol for the query selector.Default is =. Example: LIKE (will create LIKE selecting query)</span>
                
                <p>Example: $Query->remove("3", "id", "web_page");</p>
                <span>Will remove a row from web_page table with id = '3' woth the following query: DELETE FROM web_page WHERE id='3'.</span>
                <p>Example: $Query->remove("homepage", "type", "web_page", "LIKE");</p>
                <span>Will remove a row from web_page table with type LIKE 'homepage' woth the following query: DELETE FROM web_page WHERE type LIKE 'homepage'.</span>
            </li>
        </ul>
    <h3>Query->loop</h3>
        ...
        <ul>
            <li>
                ...
                <p>Example: $Query->loop("SELECT * FROM web_page WHERE tag='web'");</p>
    ...
    <h3>Query->cleanup</h3>
        ...
        <ul>
            <li>
                ...
                <p>Example: $Query->cleanup("3", "id", "web_page");</p>
                ...
                <p>Example: $Query->cleanup("homepage", "type", "web_page", "LIKE");</p>
    ...
    <h3>Query->column_group</h3>
        ...
        <ul>
            <li>
                ...
                <p>Example: $Query->column_group("type", "web_page");</p>
    ...
    <h3>Query->table</h3>
        ...
        <ul>
            <li>
                ...
                <p>Example: $Query->table("page");</p>
                <span>Will return prefix_page.</span>
                <p>Example: $Query->table();</p>
                <span>Will return default table name if current url is located in module or plugin.</span>
            </li>
        </ul>
    <h3>Query->new_id</h3>
        <span>Return next available id for specified table.</span>
        <span>This function can be applied only for integer values.</span>
        <ul>
            <li>
                <h4>Query->new_id(table=false, column=id, where clause="")</h4>
                <span>table (optional): the table to select new_id for. If left unfilled and the current url location is in some module or plugin, the system will automatically resolve the default module or plugin table. Example: web_page (will output new id value for table web_page)</span>
                <span>column (optional): the column where the next integer value will be searched. Default is id. Exaple: row</span>
                <span>where clause (optional): where clause to add to the auto generated query. Exaple: WHERE type='homepage'</span>
                
                <p>Example: $Query->new_id("web_page");</p>
                <span>Will return higher integer value + 1 for the id column in web_page table.</span>
            </li>
        </ul>
    <h3>Query->top_id</h3>
        ...
        <ul>
            <li>
                ...
                <p>Example: $Query->top_id(3, "link_id", "id", "web_page");</p>
