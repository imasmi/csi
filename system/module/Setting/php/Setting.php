...
class Setting{
    ...
    public function __construct($page_id=false, $array = array()){
        ...
        $this->table = (isset($array["table"])) ? $array["table"] : $Query->table("setting");
    ...
    public function update_setting($setting, $field, $array){?>
        <?php 
        $style = 'class="Setting"';
        $json_setting = "{page_id: '" . $setting["page_id"] . "', link_id: '" . $setting["link_id"] . "', tag:'" . $setting["tag"] . "', plugin: '" . (isset($array["plugin"]) ? $array["plugin"] : $this->plugin) . "', " . $field . ": " . ($array["update-input"] == "checkbox" ? "(this.checked === true ? '1' : '')" : "this.value") . "}";
        
        if($array["update-input"] == "text" || $array["update-input"] == "number" || $array["update-input"] == "checkbox"){
        ?>
            <input type="<?php echo $array["update-input"];?>" 
            <?php echo $style;?> 
            <?php if($array["update-input"] != "checkbox"){?> value="<?php echo $setting[$field];?>" <?php } elseif($setting[$field] == "1") { ?> checked <?php } ?>
            onchange="S.post('<?php echo $this->Core->url();?>Setting/query/admin/update-setting', <?php echo $json_setting;?>, function(data){if(data != 'done'){S.popup(data);}})"/>
        <?php
        } elseif($array["update-input"] == "textarea"){
        ?>
            <textarea <?php echo $style;?> onchange="S.post('<?php echo $this->Core->url();?>Setting/query/admin/update-setting', <?php echo $json_setting;?>, function(data){if(data != 'done'){S.popup(data);}})"><?php echo $setting[$field];?></textarea>
        <?php
        }
    }
    ...
    public function _($tag, $array=array()){
        ...
        if(isset($array["update-input"]) && $this->User->_() == "admin"){
            $update = "";
            if(empty($setting)){
                $setting = array(
                    "page_id" => isset($array["page_id"]) && is_numeric($array["page_id"]) ? $array["page_id"] : $this->page_id,
            ...
            if(isset($array["column"]) && $array["column"] != ""){
                if($array["column"] == "*"){
                    ...
                    return $setting[$array["column"]];
                }
            } else {
                if(isset($setting[$this->Language->_()])){
                    return $setting[$this->Language->_()];
                } else {
...
$Setting = new Setting($Page->id, array("page_id" => 0));
?>