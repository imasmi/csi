...
$actions = array(
    ...
    echo $check_subpages->rowCount() > 0 ? \'<button type="button" class="button" onclick="window.open(\\\'' . $Core->this_path() . '?' . (isset($_GET["menu"]) ? 'menu=' . $_GET["menu"] . '&' : '') . 'link_id=\' . $list["id"] . \'\\\', \\\'_self\\\')">Subpages</button>\' : "N/A";'),
    "delete" => $dir . "/delete"
);
...
<?php if(!isset($_GET["menu"])){?><button class="button" onclick="window.open('<?php echo $Core->this_path(0,-1);?>/menu/index', '_self')">Menus</button><?php } ?>
<?php 
...
$ListingAPP = new \system\module\Listing\php\ListingAPP($array);
