...
$text = $Query->select($_GET["id"]);
$Text = new \system\module\Text\php\Text($text["page_id"], array("fortable" => $text["fortable"], "page_id" => $text["page_id"]));
?>
...
<div class="title text-center edit-title">Edit text: <?php echo $text["tag"]?></div>
    ...
    <div class="padding-20">Advanced: <span class="inline-block"><?php $Form->on_off("addText", 0, false);?></span></div>
    <form class="form hide" id="addText" action="<?php echo $Core->query_path()  . "?id=" . $text["id"];?>" method="post" onsubmit="return S.post('<?php echo $Core->query_path() . "?id=" . $text["id"];?>', S.serialize('#addText'), '#error-message')">
