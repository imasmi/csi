...
class Text{
    ...
    public function __construct($page_id=false, $array = array()){
        ...
        $this->table = (isset($array["table"])) ? $array["table"] : $Query->table("text");
    ...
    public function items(){
        ...
        $page_id = isset($this->arr["page_id"]) ? " (page_id='" . $this->arr["page_id"] . "' OR page_id='" . $this->page_id . "') " : " page_id='" . $this->page_id . "' ";
        foreach($this->PDO->query("SELECT *  FROM " . $this->table .  " WHERE " . $page_id . " AND " . $fortable . " ORDER by id ASC") as $text){
            $outText = ($text[$this->Language->_()] != "") ? $text[$this->Language->_()] : $text["tag"];
            $texts[$text["id"]] = $text;
            $texts[$text["tag"]] = $text;
        }
        return $texts;
    ...
    public function id($id, $array=false){
        ...
        if(http_response_code() != '404'){
            ...
            if($check_tag->rowCount() > 0 && !isset($array["page_id"]) && (isset($this->arr["page_id"]) && $this->arr["page_id"] == 0)){
                $tag = $check_tag->fetch();
                $this->Query->update(array("page_id" => "0"), $tag["id"], "id", $this->table);
    ...
    public function insert_text($text){
        ...
        $new_text = array("id" => $id, "tag" => $text, "page_id" => $this->page_id, "fortable" => $this->fortable, "plugin" => $this->plugin, $this->Language->_() => NULL);
	...
	public function edit_text($id){
		if($this->User->_() == "admin"){
			return ' onmousedown="TextAPP.showWysiwyg(event, ' .$id . ')" contenteditable id="text-edit-' . $id . '"';
		}
	}
	
	public function lang_editor($id){
        if(count($this->Language->items) > 1 && $this->User->_() == "admin"){
	        $output = '<div class="language-editor" id="language-editor-' . $id . '">';
                foreach($this->Language->items as $key => $value){
                    $selected_language = ($value == $this->Language->_()) ? ' selected-text-lang' : "";
                    $output .= '<span class="lang-changer' . $selected_language . '" id="lang-changer-' . $value . '-' . $id . '" onclick="TextAPP.langChange(\'' . $id . '\', \'' . $value . '\')">' . $key . '</span>';
                }
                $output .= '<input type="hidden" id="text-lang-' . $id . '" value="' . $this->Language->_() . '"/>';
            $output .= '</div>';
            return $output;
        } elseif($this->User->_() == "admin") {
	      return '<input type="hidden" id="text-lang-' . $id . '" value="' . $this->Language->_() . '"/>';
        } else {
            return false;
        }
	}
	...
	public function item($text, $array=array()){
        ...
        if(isset($this->items[$id])){
            return $this->items[$id][$this->Language->_()] == NULL  && $this->items[$id]["page_id"] == 0 && !$array["strict"] ? $this->items[$id]["tag"] : $this->items[$id][$this->Language->_()];
        }
	...
	public function _($text, $array=array()){
	    $id = $this->id($text, $array);
	    $output = "";
	    if($this->User->_() == "admin"){ $output .= '<div class="text-wrapper inline-block">';}
		...
		$output .= "</div>";
		if(!empty($array) && $this->User->_() == "admin"){
...
$Text = new Text($Page->id, array("page_id" => 0));
?>